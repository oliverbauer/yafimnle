...
public class FFMpegScriptAudio extends FFMpegScript {
    ...
    public void fade(List<Builder> builders) {
        if (builders.size() > 1) {
            ...
            stringBuilder().append("  -vsync vfr -acodec aac -map \"[a]\" -y -b:a 192k -ar 44100 -ac 2 ").append(config.ffmpeg().threads()).append(" ").append(aacName).append("\n");
        } else {
            // only one input, no filter_complex
            stringBuilder().append("  -vsync vfr -acodec aac -y -b:a 192k -ar 44100 -ac 2 ").append(config.ffmpeg().threads()).append(" ").append(aacName).append("\n");
        }
