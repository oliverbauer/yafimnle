...
public class FFProbe {
    ...
    public Resolution resolution(File file) {
        ...
        var i = FileUtils.escapeWhitespaces(file);

        var dimension =  CLI.exec("ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=p=0:s=x " + i, this);
    ...
    public String length(File file) {
        var i = FileUtils.escapeWhitespaces(file);
        return CLI.exec("ffprobe -v error -select_streams v:0 -show_entries stream=duration -of default=noprint_wrappers=1:nokey=1 " + i, this);
    }
